...
def benchmark_model_preds(
    X, Q, pos_pred, rot_pred, x_mean, x_std, offsets, parents, out_path=None, n_past=10, n_future=10
):
    """
    Evaluate naive baselines (zero-velocity and interpolation) for transition generation on given data.
    :param X: Local positions array of shape (Batchsize, Timesteps, Joints, 3)
    ...
    for n_trans in trans_lengths:
        ...
        trans_rmib_global_poses = (pos_pred[:,:n_trans].reshape(pos_pred.size(0), n_trans, -1).permute(0,2,1).numpy() - x_mean) / x_std
        ...
        res[("RMIB_quat_loss", n_trans)] = np.mean(
            np.sqrt(
                np.sum(
                    (rot_pred[:,:n_trans].numpy() - trans_gt_global_quats) ** 2.0,
                    axis=(2, 3),
                )
            )
        )
        ...
        res[("RMIB_pos_loss", n_trans)] = np.mean(
            np.sqrt(
                np.sum(
                    (trans_rmib_global_poses - trans_gt_global_poses) ** 2.0, axis=1
                )
            )
        )
    ...
    avg_zerov_quat_losses = [res[("zerov_quat_loss", n)] for n in trans_lengths]
    avg_interp_quat_losses = [res[("interp_quat_loss", n)] for n in trans_lengths]
    avg_rmib_quat_losses = [res[("RMIB_quat_loss", n)] for n in trans_lengths]
    print("=== Global quat losses ===")
    print(
        "{0: <16} | {1:6d} | {2:6d} | {3:6d}".format("Lengths", 5, 15, 30)
    )
    print(
        "{0: <16} | {1:6.2f} | {2:6.2f} | {3:6.2f}".format(
            "Zero-V", *avg_zerov_quat_losses
        )
    )
    print(
    ...
    avg_zerov_pos_losses = [res[("zerov_pos_loss", n)] for n in trans_lengths]
    avg_interp_pos_losses = [res[("interp_pos_loss", n)] for n in trans_lengths]
    avg_rmib_pos_losses = [res[("RMIB_pos_loss", n)] for n in trans_lengths]
    print("=== Global pos losses ===")
    print(
        "{0: <16} | {1:6d} | {2:6d} | {3:6d}".format("Lengths", 5, 15, 30)
    )
    print(
        "{0: <16} | {1:6.3f} | {2:6.3f} | {3:6.3f}".format(
            "Zero-V", *avg_zerov_pos_losses
        )
    )
    print(
    ...
    print("=== NPSS on global quats ===")
    print(
        "{0: <16} | {1:5d}  | {2:5d}  | {3:5d}".format(
            "Lengths", 5, 15, 30
        )
    )
    print(
        "{0: <16} | {1:5.4f} | {2:5.4f} | {3:5.4f}".format(
            "Zero-V", *avg_zerov_npss_losses
        )
    )
    print(
