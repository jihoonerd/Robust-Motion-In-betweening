...
class LAFAN1Dataset(Dataset):
    ...
    def __init__(self, lafan_path: str, train: bool, device: str):
        ...
        self.actors = ['subject1', 'subject2', 'subject3', 'subject4'] if train else ['subject5']
    ...
    def load_lafan(self):
        ...
        X, Q, parents, contacts_l, contacts_r = extract.get_lafan1_set(self.lafan_path, self.actors, self.window, self.offset)
        ...
        input_data['root_v'] = global_pos[:, 1:, 0, :] - global_pos[:,:-1, 0, :]  # \dot{r}_{t}
        input_data['root_p_offset'] = global_pos[:,-1,0,:]  # last frame's root positions
        input_data['root_p'] = global_pos[:,:,0,:]

        input_data['contact'] = np.concatenate([contacts_l, contacts_r], -1) # Foot contact
        input_data['global_pos'] = global_pos[:,:,:,:] # global position (N, 50, 22, 30) why not just global_pos
        return input_data
